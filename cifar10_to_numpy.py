...
tf.app.flags.DEFINE_string('data_dir', '/home/wjf/datasets/CIFAR10', 'where to store the dataset')
...
tf.app.flags.DEFINE_integer('dataset_seed', 123, "dataset seed")
...
def prepare_dataset():
    ...
    dirpath = os.path.join(FLAGS.data_dir, 'SSL/seed' + str(FLAGS.dataset_seed))
    ...
    examples_per_class = int(FLAGS.num_valid_examples / 10)
    valid_images = np.zeros((FLAGS.num_valid_examples, 3072), dtype=np.float32)
    valid_labels = np.zeros((FLAGS.num_valid_examples), dtype=np.int64)
    for i in xrange(10):
        ind = np.where(_train_labels == i)[0]
        valid_images[i * examples_per_class:(i + 1) * examples_per_class] \
            = _train_images[ind[0:examples_per_class]]
        valid_labels[i * examples_per_class:(i + 1) * examples_per_class] \
            = _train_labels[ind[0:examples_per_class]]
        _train_images = np.delete(_train_images,
                                     ind[0:examples_per_class], 0)
        _train_labels = np.delete(_train_labels,
    ...
    np.savez(os.path.join(dirpath, 'labeled_train.npz'), image=labeled_train_images, label=labeled_train_labels)
    np.savez(os.path.join(dirpath, 'unlabeled_train.npz'), image=train_images, label=train_labels)
    np.savez(os.path.join(dirpath, 'valid.npz'), image=valid_images, label=valid_labels)
    np.savez(os.path.join(dirpath, 'test.npz'), image=test_images, label=test_labels)
